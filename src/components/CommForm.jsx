...
const CommForm = (props) => {
  const { onClose, onCommUpdate, id } = props;
  ...
  return (
    <div style={{fontSize: 13, fontWeight: '600', width: 500 }}>
      ...
      <div className='row'>
        ...
        <input className='modal-input' type="text" value={comm} style={{minHeight: 100, marginLeft: 0, width: '90%'}} onChange={e=>setComm(e.target.value)} />
      </div>
      <div className='row' style={{ marginBottom: 10 }}>
        <button onClick={()=>{onClose(); onCommUpdate(comm, id);}} className='secondary-button'>
          Mark Communication
        </button>
      </div>
    </div>
